...
> **Feature path:** `features/dashboard/graphql/`  
> **Halaman:** `/dashboard/recommendation`  
> **Komponen utama:** `HashtagCloud.tsx`, `TrendingPostCard.tsx`

### Queries

| Operasi | Input | Return Type | File | Dipakai di |
|---------|-------|-------------|------|------------|
| `topHashtags` (GetTopHashtags) | `Platform!`, `Int` | `TopHashtag[]` | [top-hashtags.query.ts](../features/dashboard/graphql/top-hashtags.query.ts) | `HashtagCloud` |
| `topTrending` (GetTopTrending) | `Platform!` | `TrendingPost[]` | [trending-posts.query.ts](../features/dashboard/graphql/trending-posts.query.ts) | `TrendingPostCard` |

### Return Fields (TopHashtag)
...
- Query `topHashtags` menerima parameter `platform` (required) dan `limit` (default: 10). Data digunakan oleh `HashtagCloud` untuk menampilkan daftar hashtag trending per platform.
- Query `topTrending` menerima parameter `platform` (required) dan mengembalikan daftar trending posts dari platform tersebut. Digunakan oleh komponen `TrendingPostCard` untuk menampilkan post-post yang sedang trending.
...
│                        RECOMMENDATION                                │
│                                                                     │
│  HashtagCloud ─────── GetTopHashtags (Q)                            │
│  TrendingPostCard ──── GetTopTrending (Q)                           │
├─────────────────────────────────────────────────────────────────────┤
│                        PROFILE                                      │
│                                                                     │
...
| [recommendation.types.ts](../features/dashboard/graphql/recommendation.types.ts) | `TopHashtag`, `Recommendation`, `BestTimeToPost`, `TrendingPost` | Recommendation queries |
...
| `top-hashtags.query.ts` | Query | `GetTopHashtags` |
| `trending-posts.query.ts` | Query | `GetTopTrending` |
